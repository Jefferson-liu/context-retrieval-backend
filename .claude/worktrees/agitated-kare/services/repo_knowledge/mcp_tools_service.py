...
class RepoKnowledgeMcpService:
    ...
    async def get_run_overview(self, *, run_id: str) -> dict:
        ...
        latest_repo_manager = await self.repo_manager_run_repo.latest_completed_for_source(
            source_run_id=run.id,
            tenant_id=self.tenant_id,
            user_id=self.user_id,
        )
    ...
    async def read_repo_manager_segment(
        ...
        *,
        ...
        row = await self.segment_repo.get_group_with_summary(
            repo_manager_run_id=summary_run.id,
            group_id=group_id,
            group_key=group_key,
        )
    ...
    async def _resolve_repo_manager_run(
        ...
        *,
        ...
        if repo_manager_run_id:
            summary_run = await self.repo_manager_run_repo.get_scoped(
                repo_manager_run_id,
                tenant_id=self.tenant_id,
                user_id=self.user_id,
            )
            ...
            return summary_run
        return await self.repo_manager_run_repo.latest_completed_for_source(
            source_run_id=run.id,
            tenant_id=self.tenant_id,
            user_id=self.user_id,
        )
    ...
    @staticmethod
    def _serialize_repo_manager_run(run: RepoManagerRunRecord | None) -> dict | None:
        if run is None:
            return None
        return {
