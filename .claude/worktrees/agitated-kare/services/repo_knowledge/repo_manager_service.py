...
from services.repo_knowledge.architecture.errors import TraceableLLMError
from services.repo_knowledge.architecture.architecture_merger import (
...
class RepoKnowledgeRepoManagerService:
    ...
    def __init__(self, session: AsyncSession, *, tenant_id: str, user_id: str) -> None:
        ...
        self.repo_manager_run_repo = RepoManagerRunRepository(session)
        self.segment_repo = RepoManagerSegmentRepository(session)
    ...
    async def create_repo_manager_run(
        ...
        if file_summary_run is None:
            raise RepoManagerError("source_file_summary_run_id not found")
        if file_summary_run.status != "completed":
            raise RepoManagerError("source_file_summary_run_id must be completed")
        ...
        if source_run is None:
            raise RepoManagerError("source_run_id not found")
        if source_run.status not in {"completed", "skipped_duplicate"}:
            raise RepoManagerError("source_run_id must be completed or skipped_duplicate")

        run_fingerprint = _build_repo_manager_fingerprint(
            source_run_id=source_run.id,
            source_file_summary_run_id=file_summary_run.id,
            prompt_version=settings.REPO_MANAGER_PROMPT_VERSION,
        ...
        if not payload.force_rerepo_manager:
            existing = await self.repo_manager_run_repo.find_completed_by_fingerprint(
                source_run_id=source_run.id,
                source_file_summary_run_id=file_summary_run.id,
                tenant_id=self.tenant_id,
                user_id=self.user_id,
            ...
            if existing is not None:
                logger.info(
                    "Reusing completed repo-manager run repo_manager_run_id=%s source_run_id=%s file_summary_run_id=%s",
                    existing.id,
                    source_run.id,
                    file_summary_run.id,
                )
        ...
        run = await self.repo_manager_run_repo.create(
            source_run_id=source_run.id,
            source_file_summary_run_id=file_summary_run.id,
            tenant_id=self.tenant_id,
            user_id=self.user_id,
        ...
        logger.info(
            "Created repo-manager run repo_manager_run_id=%s source_run_id=%s file_summary_run_id=%s",
            run.id,
            run.source_run_id,
    ...
    async def get_repo_manager_run_status(self, *, repo_manager_run_id: str) -> RepoManagerRunRecord | None:
        return await self.repo_manager_run_repo.get_scoped(
            repo_manager_run_id,
            tenant_id=self.tenant_id,
            user_id=self.user_id,
        )

    async def get_architecture_artifact(self, *, repo_manager_run_id: str) -> RepoManagerRunRecord | None:
        run = await self.repo_manager_run_repo.get_scoped(
            repo_manager_run_id,
            tenant_id=self.tenant_id,
            user_id=self.user_id,
        )
    ...
    async def list_repo_manager_segments(
        ...
        *,
        ...
        rows = await self.segment_repo.list_groups_for_run(
        ...
        if include_members:
            ...
            member_map = await self.segment_repo.list_members_for_groups(
                repo_manager_run_id=run.id,
                group_ids=group_ids,
            )
    ...
    async def latest_repo_manager_segments_for_source_run(
        ...
        *,
        ...
        latest = await self.repo_manager_run_repo.latest_completed_for_source(
            source_run_id=source_run.id,
            tenant_id=self.tenant_id,
            user_id=self.user_id,
        )
...
class RepoManagerWorker:
    ...
    def __init__(self, session: AsyncSession) -> None:
        ...
        self.segment_repo = RepoManagerSegmentRepository(session)
        self.segment_summary_repo = RepoManagerSegmentSummaryRepository(session)
        self.diagnostic_repo = RepoManagerDiagnosticRepository(session)

    async def execute(self, *, repo_manager_run_id: str) -> None:
        settings = get_settings()
        logger.info("Repo manager run start repo_manager_run_id=%s", repo_manager_run_id)

        run = await self.repo_manager_run_repo.get(repo_manager_run_id)
        if run is None:
            logger.warning("Repo manager run abort: run not found repo_manager_run_id=%s", repo_manager_run_id)
            return
        if run.status not in {"queued", "failed"}:
            logger.info("Skipping repo-manager run %s with status=%s", run.id, run.status)
            return

        run_id_value = run.id
        await self.repo_manager_run_repo.mark_in_progress(run)
        await self.session.commit()

        source_run = await self.run_repo.get(run.source_run_id)
        if source_run is None:
            await self.repo_manager_run_repo.mark_failed(run, error_message="source_run_id not found")
            await self.session.commit()
            return
        if source_run.status not in {"completed", "skipped_duplicate"}:
            await self.repo_manager_run_repo.mark_failed(
                run,
        ...
        if file_summary_run is None:
            await self.repo_manager_run_repo.mark_failed(run, error_message="source_file_summary_run_id not found")
            await self.session.commit()
            return
        if file_summary_run.status != "completed":
            await self.repo_manager_run_repo.mark_failed(
        ...
        try:
            ...
            await self.repo_manager_run_repo.mark_failed(run, error_message=str(exc))
            await self.session.commit()
            return
        ...
        try:
            ...
            await self.repo_manager_run_repo.set_counts(
                run,
            ...
            await self.segment_repo.clear_for_run(repo_manager_run_id=run.id)
            ...
            await self.segment_repo.create_groups(group_rows)
            await self.segment_repo.create_group_members(member_rows)
            await self.session.commit()

            neighbor_paths = _segment_neighbor_paths(
            ...
            for group in build_result.groups:
                try:
                    ...
                    await self.segment_summary_repo.upsert(
                        repo_manager_run_id=run.id,
                        group_id=group.group_id,
                        source_run_id=run.source_run_id,
                        source_file_summary_run_id=run.source_file_summary_run_id,
                    ...
                    await self.diagnostic_repo.create(
                        repo_manager_run_id=run.id,
                        group_id=group.group_id,
                        severity="error",
                        message=exc.message,
            ...
            if counters.groups_summarized == 0:
                ...
                try:
                    ...
                    await self.session.commit()
                    return
                await self.repo_manager_run_repo.set_architecture_artifact(
                    run,
                ...
                await self.repo_manager_run_repo.mark_completed(run)
            await self.session.commit()
        except Exception as exc:  # pragma: no cover - run-level failures
            await self.session.rollback()
            logger.exception("Repo manager run failed repo_manager_run_id=%s", run_id_value)
            failed = await self.repo_manager_run_repo.get(run_id_value)
            if failed:
                await self.repo_manager_run_repo.mark_failed(failed, error_message=str(exc))
                await self.session.commit()
...
def _build_repo_manager_fingerprint(
    *,
    source_run_id: str,
    source_file_summary_run_id: str,
    prompt_version: str,
