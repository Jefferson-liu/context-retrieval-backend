...
from services.repo_knowledge.architecture.types import SegmentArchitectureInput, SegmentArchitectureOutput

ARCHITECTURE_SEGMENT_SYSTEM_PROMPT = load_prompt("architecture_segment_system")


@dataclass(slots=True)
class SegmentArchitectureResult:
    """Structured summary output plus raw model response payload."""

    output: SegmentArchitectureOutput
    raw_output: dict
    input_hash: str


class RepoSegmentArchitect:
    ...
    async def summarize(self, *, payload: SegmentArchitectureInput) -> SegmentArchitectureResult:
        material = [
        ...
        if not payload.members:
            ...
            return SegmentArchitectureResult(
                output=output,
    ...
    def _build_prompt(self, payload: SegmentArchitectureInput) -> str:
    ...
    async def _invoke_structured(self, *, user_prompt: str) -> tuple[SegmentArchitectureOutput, str]:
        ...
        for _ in range(self.retry_count):
            try:
                response = await self._invoke_messages(
                    messages=[
                        SystemMessage(content=ARCHITECTURE_SEGMENT_SYSTEM_PROMPT),
                        HumanMessage(content=user_prompt),
                    ]
                )
                raw_text = self._response_to_text(response)
                last_raw_text = raw_text
                payload = _normalize_segment_payload(_extract_json(raw_text))
                output = SegmentArchitectureOutput.model_validate(payload)
...
def _normalize_segment_payload(payload: dict) -> dict:
