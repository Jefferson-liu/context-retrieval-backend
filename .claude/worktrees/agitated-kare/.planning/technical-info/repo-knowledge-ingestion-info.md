...
- `POST /repo-knowledge/repo-manager-runs`
  ...
  - Storage layer now uses canonical `repo_manager` repositories/tables.
- `POST /repo-knowledge/pipeline-runs`
  - Creates a source ingestion run from `source_path` and orchestrates downstream file-summary -> repo-full-summary -> embedding -> repo-manager automatically.
...
- Services:
  ...
  - `services/repo_knowledge/repo_manager_service.py` (canonical implementation)
  - `services/repo_knowledge/repo_manager_background_runner.py` (canonical runner)
  - `services/repo_knowledge/grouping/*`
  - `services/repo_knowledge/architecture/*`
...
- Repositories:
  ...
  - `infrastructure/repositories/repo_manager_run_repository.py`
  - `infrastructure/repositories/repo_manager_segment_repository.py`
  - `infrastructure/repositories/repo_manager_segment_summary_repository.py`
  - `infrastructure/repositories/repo_manager_diagnostic_repository.py`
- Models:
...
- `main.py` lifespan now also starts/stops the repo-manager worker.
...
- Prompt source for repo-knowledge LLM operations is file-backed (no inline fallback constants):
  ...
  - architecture segment prompts: `services/repo_knowledge/prompts/architecture_segment_*.md`
...
- Implemented MCP tools for table-level inspection and summary reading:
  ...
  - `repo_list_repo_manager_segments`
  - `repo_read_repo_manager_segment`
  - `repo_structure`
