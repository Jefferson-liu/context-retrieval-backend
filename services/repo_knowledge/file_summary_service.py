...
class FileSummaryAgentConfig:
    ...
    fast_path_char_threshold: int = 3000
    ...
    def from_settings(cls) -> "FileSummaryAgentConfig":
        ...
        return cls(
            ...
            fast_path_char_threshold=settings.REPO_SUMMARY_FAST_PATH_CHAR_THRESHOLD,
        )
...
class RepoFileSummaryWorker:
    ...
    async def execute(self, *, file_summary_run_id: str) -> None:
        ...
        summarizer = RepoFileSummarizer(
            ...
            fast_path_char_threshold=cfg.fast_path_char_threshold,
        ...
        try:
            ...
            lock = asyncio.Lock()
            rate_limited_files: list[tuple] = []
            other_failed_files: list[tuple] = []
                ...
                async with semaphore:
                    ...
                    async with lock:
                        try:
                            ...
                            await self.session.commit()
                            other_failed_files.append((snapshot, subject))
                            return

                        # Hash cache: reuse an existing summary if content unchanged
                        input_hash = RepoFileSummarizer.compute_input_hash(
                            context, prompt_version=run.prompt_version
                        )
                        cached = await self.summary_repo.find_by_subject_and_hash(
                            subject_id=subject.id, input_hash=input_hash
                        )
                        if cached is not None:
                            await self.summary_repo.upsert(
                                ...
                                file_cluster=cached.file_cluster,
                                important_relationships=cached.important_relationships,
                                group_function=cached.group_function,
                                raw_output={**cached.raw_output, "cache_hit": True},
                    ...
                    try:
                        ...
                        if _is_rate_limit_error(exc):
                            rate_limited_files.append((snapshot, subject))
                        else:
                            other_failed_files.append((snapshot, subject))
                        return
            ...
            # Snapshot and clear both retry lists before running retry passes so
            # that second-pass failures don't trigger a third pass.
            rl_batch = list(rate_limited_files)
            other_batch = list(other_failed_files)
            rate_limited_files.clear()
            other_failed_files.clear()
            counters.files_failed -= len(rl_batch) + len(other_batch)

            if rl_batch:
                logger.info(
                    "Backing off 30s before retrying %d rate-limited files for run %s",
                    len(rl_batch), run_id_value,
                )
                await asyncio.sleep(30)
                await asyncio.gather(*[_process_file(snapshot, subject) for snapshot, subject in rl_batch])

            if other_batch:
                logger.info(
                    "Retrying %d failed files for run %s",
                    len(other_batch), run_id_value,
                )
                await asyncio.gather(*[_process_file(snapshot, subject) for snapshot, subject in other_batch])
...
def _is_rate_limit_error(exc: Exception) -> bool:
    raw = str(exc).lower()
    return any(kw in raw for kw in ("429", "quota", "rate limit", "too many requests", "resource_exhausted", "rateerror"))
