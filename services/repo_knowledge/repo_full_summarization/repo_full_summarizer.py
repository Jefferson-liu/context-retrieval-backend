...
class RepoFullSummarizer:
    ...
    async def summarize(self, *, payload: RepoFullSummaryInput) -> RepoFullSummaryResult:
        ...
        # Fast path: if all file summaries fit in context, skip the ReAct agent
        # and make a single direct LLM call with everything embedded inline.
        fast_path_result = await self._try_fast_path(payload=payload, user_prompt=user_prompt)
        if fast_path_result is not None:
            return RepoFullSummaryResult(
                output=fast_path_result[0],
                raw_output={"mode": "fast_path", "raw_text": fast_path_result[1]},
    ...
    async def _try_fast_path(
        ...
        *,
        payload: RepoFullSummaryInput,
        user_prompt: str,
    ) -> tuple[RepoFullSummaryOutput, str] | None:
        """Return (output, raw_text) if all summaries fit in one LLM call, else None."""

        agent_tools = RepoFullSummaryAgentTools(
            payload=payload,
        ...
        index = await agent_tools._load_summary_index()
        if not index:
            return None

        lines: list[str] = ["## File Summaries\n"]
        for path, entry in sorted(index.items()):
            formatted = _format_file_summary_result({
                "resolved_subject_path": entry["subject_path"],
                "match_type": "exact",
                "language": entry.get("language"),
                "overall_summary": entry["overall_summary"],
                "file_cluster": entry.get("file_cluster", []),
                "important_relationships": entry.get("important_relationships", []),
                "group_function": entry.get("group_function"),
            })
            lines.append(formatted)
            lines.append("")

        summaries_block = "\n".join(lines)
        if len(summaries_block) > self.max_input_chars:
            return None

        combined_user = (
            f"{user_prompt}\n\n"
            "All file summaries are provided below — no tool calls are needed.\n\n"
            f"{summaries_block}"
        )
        messages = [
            SystemMessage(content=REPO_FULL_SUMMARY_SYSTEM_PROMPT),
            HumanMessage(content=combined_user),
        ]
        response = await self.chat_model.ainvoke(messages)
        raw_text: str = response.content if isinstance(response.content, str) else str(response.content)
        logger.info(
            "repo_full_summary fast_path output (%d chars, %d files):\n%s",
            len(raw_text),
            len(index),
            raw_text[:2000],
        )
        output = RepoFullSummaryOutput(readme_markdown=raw_text.strip())
