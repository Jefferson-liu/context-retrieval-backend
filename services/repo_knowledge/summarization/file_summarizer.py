...
_FAST_PATH_INSTRUCTION = (
    "Summarize this file based on the code provided below. "
    "Focus on what the file does, its key exports/classes/functions, "
...
class RepoFileSummarizer:
    ...
    def __init__(
        ...
        *,
        ...
        self.fast_path_char_threshold = max(0, fast_path_char_threshold)
        self.subject_repo = subject_repo
        self.edge_repo = edge_repo
        log_react_agent_dependency_versions(logger)

    @staticmethod
    def compute_input_hash(payload: SummaryInput, prompt_version: str) -> str:
        material = [
    ...
    async def summarize(self, *, payload: SummaryInput) -> SummaryResult:
        input_hash = self.compute_input_hash(payload, self.prompt_version)
        ...
        # Fast path: single LLM call for small files — no ReAct agent needed
        if self.fast_path_char_threshold > 0 and sum(len(c) for c in payload.chunk_texts) < self.fast_path_char_threshold:
            import time as _time
            t0 = _time.perf_counter()
            raw_text, usage_trace = await self._synthesize_without_tools(
                user_prompt, instruction=_FAST_PATH_INSTRUCTION
            )
            usage_trace["fast_path"] = True
            usage_trace["duration_ms"] = int((_time.perf_counter() - t0) * 1000)
            output = SummaryOutput.model_validate(_extract_summary_payload(raw_text))
            return SummaryResult(
                output=output,
                raw_output={"mode": "fast_path", "raw_text": raw_text},
                input_hash=input_hash,
    ...
    async def _synthesize_without_tools(
        self, user_prompt: str, *, instruction: str = _SYNTHESIS_FALLBACK_INSTRUCTION
    ) -> tuple[str, dict]:
        """Single LLM call without tools — used for fallback and small-file fast path."""
