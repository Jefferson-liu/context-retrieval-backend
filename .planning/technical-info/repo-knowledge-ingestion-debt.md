...
- Repo Manager/architecture runs still store artifacts in the legacy `group_summary` tables; no retention/compaction policy is implemented for stale/obsolete runs and a naming cleanup migration is still needed.
...
- Repo Manager/architecture artifacts now have a canonical `repo_manager` API/module surface, but persistence still lives on legacy `group_summary` tables/repositories; a formal DB/domain cleanup migration is still needed if the storage layer should match the public vocabulary.
...
- MCP tooling now exposes repo-manager terminology for segment inspection, but old `group_summary` naming may still appear in legacy downstream consumers until they are updated.
