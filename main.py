...
from services.repo_knowledge.agent_mcp_app import mcp as agent_repo_knowledge_mcp
from services.repo_knowledge.mcp_app import mcp as repo_knowledge_mcp
...
_agent_mcp_session_manager: StreamableHTTPSessionManager | None = None


@asynccontextmanager
async def lifespan(app: FastAPI):
    global _mcp_session_manager, _agent_mcp_session_manager
    await init_db()
    _mcp_session_manager = StreamableHTTPSessionManager(
        app=repo_knowledge_mcp._mcp_server,
        event_store=None,
        json_response=False,
        stateless=False,
    )
    _agent_mcp_session_manager = StreamableHTTPSessionManager(
        app=agent_repo_knowledge_mcp._mcp_server,
        event_store=None,
        json_response=False,
        stateless=False,
    )
    async with _mcp_session_manager.run(), _agent_mcp_session_manager.run():
...
async def _mcp_asgi(scope, receive, send):
    if _mcp_session_manager is not None:
        ...
        response = PlainTextResponse("MCP not ready", status_code=503)
        await response(scope, receive, send)


async def _agent_mcp_asgi(scope, receive, send):
    if _agent_mcp_session_manager is not None:
        await _agent_mcp_session_manager.handle_request(scope, receive, send)
    else:
        from starlette.responses import PlainTextResponse
        response = PlainTextResponse("Agent MCP not ready", status_code=503)
        await response(scope, receive, send)


app.mount("/mcp", _mcp_asgi)
app.mount("/agent-mcp", _agent_mcp_asgi)
