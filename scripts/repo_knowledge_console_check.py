...
def run() -> int:
    ...
    try:
        ...
        record("group_summarizer_schema", bool(out.output.name) and out.output.confidence >= 0.0, out.output.name)
    except Exception as exc:
        record("group_summarizer_schema", False, f"exception={exc}")
